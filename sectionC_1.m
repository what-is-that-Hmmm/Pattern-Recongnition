...
PV_black_foam = [PVT_black_foam.pressure; PVT_black_foam.vibration];
PV_car_sponge = [PVT_car_sponge.pressure; PVT_car_sponge.vibration];

figure
hold on 
[PV_merged,PV_mean,mean_1,mean_2,Plt_bf,Plt_cs] = data_prepare(PV_black_foam,PV_car_sponge);

% LDA process
[feature_PV,LDA_ax] = LDA_PVT(PV_black_foam, PV_car_sponge,2);

% projecting the original data
new = (PV_merged' * feature_PV)';
% plot the new data
Plt_bf = scatter(new(1,1:10), new(2,1:10), ...
    30,[1 0 0.5],'filled');
Plt_cs = scatter(new(1,11:20), new(2,11:20), ...
    30,[0 0 0],'filled');
...
legend([Plt_bf Plt_cs mean1 mean2 general_mean liner], {'black foam', 'car sponge' 'mean for black foam', 'mean for car sponge', ...
    'overall mean' 'LDA'});
axis equal
hold off
...
PV_black_foam = [PVT_black_foam.pressure; PVT_black_foam.temperature];
PV_car_sponge = [PVT_car_sponge.pressure; PVT_car_sponge.temperature];
...
PV_black_foam = [PVT_black_foam.temperature; PVT_black_foam.vibration];
PV_car_sponge = [PVT_car_sponge.temperature; PVT_car_sponge.vibration];
...
%%  3D LDA to PVT data
pvt_bf = [PVT_black_foam.pressure; PVT_black_foam.vibration; PVT_black_foam.temperature];
pvt_cs = [PVT_car_sponge.pressure; PVT_car_sponge.vibration; PVT_car_sponge.temperature];
merged_PVT = [pvt_bf, pvt_cs];
...
% plot LDA results
Plt_bf = scatter3(new_PVT(1,1:10), new_PVT(2,1:10), new_PVT(3,1:10),...
    30,[1 0 0.5],'filled');
Plt_cs = scatter3(new_PVT(1,11:20), new_PVT(2,11:20), new_PVT(3,1:10),...
    30,[0 0 0],'filled');
...
legend([Plt_bf Plt_cs LDA1 LDA2 plane], {'black foam', 'car sponge' 'LDA1' ...
    'LDA2' 'split plane'});
hold off
